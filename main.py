...
def download_shiki_choose_translation(serv, id):
    if serv == "sh":
        ...
        return render_template('info.html',
                               title=name, image=pic, score=score, translations=serial_data['translations'],
        ...
        try:
            ...
            """
        return render_template('info.html',
                               title="...", image=config.IMAGE_NOT_FOUND, score="...",
                               translations=serial_data['translations'], series_count=serial_data["series_count"],
                               id=id,
...
@app.route(
    '/watch/<string:serv>/<string:id>/<string:data>/<int:seria>/<string:old_quality>/<int:timing>/q-<string:quality>/')
...
def watch(serv, id, data, seria, quality="720", timing=0):
    try:
        ...
        return render_template('watch.html',
                               url=url, seria=seria, series=series, id=id, id_type=id_type, data="-".join(data),
                               quality=quality, serv=serv, straight_url=straight_url,
...
def room(rid):
    ...
    try:
        ...
        return render_template('room.html',
                               url=url, seria=seria, series=series, id=id, id_type=id_type,
...
@app.route(
    '/fast_download_act/<string:id_type>-<string:id>-<int:seria_num>-<string:translation_id>-<string:quality>-<int:max_series>/')
def fast_download_work(id_type: str, id: str, seria_num: int, translation_id: str, quality: str, max_series: int = 12):
    ...
    if config.USE_SAVED_DATA and ch.is_id('sh' + id):
        if seria_num != 0:
            fname = str(ch.get_data_by_id('sh' + id)[
                            'title']) + '-' + f'Серия-{str(seria_num).zfill(add_zeros)}-Перевод-{translation}-{quality}p'
        else:
            fname = str(ch.get_data_by_id('sh' + id)['title']) + '-' + f'Перевод-{translation}-{quality}p'
        metadata = {
            'title': ch.get_data_by_id('sh' + id)['title'] + ' - Серия-' + str(seria_num) if seria_num != 0 else
            ch.get_data_by_id('sh' + id)['title'],
    ...
    try:
        ...
        return abort(500,
                     'Внимание, на сервере не установлен ffmpeg или программа не может получить к нему доступ. Ffmpeg обязателен для использования быстрой загрузки. (Стандартная загрузка работает без ffmpeg)')
...
@app.route(
    '/fast_download/<string:id_type>-<string:id>-<int:seria_num>-<string:translation_id>-<string:quality>-<int:max_series>/')
...
def on_join(data):
    ...
    return send({'data': {'status': 'loading', 'time': watch_manager.get_room_data(data['rid'])['play_time']}},
                to=data['rid'])
